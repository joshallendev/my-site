<script lang="ts">
	...
	const nonActiveLinkStyles: string = 'nav-link mx-2';
	const activeLinkStyles: string =
		'nav-link mx-2 font-semibold border-b-4 border-cyan border-solid';
	type VisibleSection = {
		elementID: string;
		pctVisible: number;
	};
...
<header
	bind:this={header}
	id="header"
	class="body-font fixed bg-richblackfogra w-full text-platinum"
>
	<div class="container mx-auto flex flex-wrap py-5 flex-col md:flex-row items-center">
		...
		<nav class="md:ml-auto flex flex-wrap items-center text-base justify-center">
			{#each navLinks as navLink (navLink.path)}
				<a
					href={navLink.path}
					class={navLink.name === activeAnchor ? activeLinkStyles : nonActiveLinkStyles}
					>{navLink.name}</a
				>
			{/each}
		</nav>
